...
{
    // r: low 4 bits for foreground color, high 4 bits for background color
	return fg | bg << 4;
}

static inline uint16_t vga_entry(unsigned char uc, uint8_t color) 
{
    // r: low 8 bits for a character, 8 high bits for the color (bg + fg)
...
void terminal_initialize(void)
{
	...
	terminal_buffer = (uint16_t*) 0xB8000; // r: VGA Text mode buffer is located at 0xB8000
